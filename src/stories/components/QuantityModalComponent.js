...
export default class QuantityModalComponent extends React.Component {
        ...
        if (this.state.quantity === "0") {
            ...
            this.setState({
                ...
                commission_amount: (
                    ...
                    100 *
                    (this.state.discount ? parseInt(this.state.discount,10) * (parseInt(this.state.price, 10) * parseInt(this.state.quantity, 10)) : (parseInt(this.state.price, 10) * parseInt(this.state.quantity, 10)) )
                ).toString(),
            });
        }
    }
    ...
    computeCommission(value){
        let commissionValue = this.props.attendants.filter(attendant => attendant._id === value);

        let priceQty = parseFloat(this.state.price,10) * parseFloat(this.state.quantity,10);
        let discountValue = parseFloat(this.state.discount) > 0 ? (priceQty) - ((parseFloat(this.state.discount) / 100) * (priceQty)) :  (priceQty);

        this.setState({
            attendantName: value,
            commission_amount: ((parseFloat(commissionValue[0].commission, 10) / 100) * discountValue).toString()
        });
    }
    render() {
                                ...
                                onNumberPress={text =>
                                    this.state.status === "Qty"
                                        ...
                                        : this.state.status === "Commission"
                                            ...
                                            : this.state.status === "Discount" ? this.onNumberDiscountPress(text) : ""
                                }
                                onDeletePress={() =>
                                    this.state.status === "Qty"
                                        ? this.onDeletePress()
                                        ...
                                        : this.state.status === "Commission"
                                            ...
                                            : this.state.status === "Discount" ? this.onDeleteDiscountPress() : ""
                                }
                            />
                        </View>
