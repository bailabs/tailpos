...
export default class ShiftContainer extends React.Component {
  ...
  payInClick(money) {
    if (
      ...
      // End cash
      defaultShift.setEndCash(defaultShift.ending_cash + parseFloat(money));
  ...
  payOutClick(money) {
    if (
      ...
      const newCash = defaultShift.ending_cash - parseFloat(money);

      if (defaultShift.ending_cash < parseFloat(money)) {
        Toast.show({
