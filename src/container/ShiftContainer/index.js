...
export default class ShiftContainer extends React.Component {
    ...
    payInClick(money) {
        if (this.props.shiftStore.defaultShift.attendant === this.props.attendantStore.defaultAttendant.user_name) {
            if (money.reason){
                this.setState({ pay: "" });
                ...
                // End cash
                defaultShift.setEndCash(defaultShift.ending_cash + parseFloat(money.pay));
                if (this.props.printerStore.rows.length > 0){
                    for (let i = 0; i < this.props.printerStore.rows.length; i += 1) {
                        if (this.props.printerStore.rows[i].defaultPrinter) {
                            ...
                            BluetoothSerial.connect(this.props.printerStore.rows[i].macAddress).then(() => {
                                const writePromises = [];
